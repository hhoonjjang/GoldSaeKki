...
> ### 핵심 기능
> * animation이 적용된 메뉴바 : 유저가 마우스를 댄 카테고리로 현재 bar의 위치와 width 변경
> 　　<img style="width : 600px" src = "https://cdn.imweb.me/upload/S2020090710444c43a5dc5/c0a943d178a64.gif">
> * navigation component : 네비게이션 바를 컴포넌트화 하여 랭킹, 커뮤니티, 고객지원 등 여러 페이지에 이식시켜 사용
...
> ### 네비바 기본 기능
> - 스크롤 일정이상 내릴시 상단 고정
> * category별 router 주소 : name과 label을 설정해 props로 보내주어 카테고리 클릭시 현재 링크의 해당 router element로 이동시킨다.
> ```
>    // props
...
> ### 네비바 이슈 사항 
>1. 카테고리 클릭시 카테고리 엘리먼트의 width를 가져와 아래 바의 크기를 변경시켜야 하는데 React의 리랜더링 문제로 width크기가 한번씩 밀려서 바로 적용안되는 문제 --> useEffect()를 사용해 state의 값이 변경될 때 리랜더링 되도록 설정해 해결할 수 있었다.
>2. left값 구하기 -> 마우스를 댄 놈의 왼쪽 element들의 width값과 margin값을 계산해 이동시키려 하였으나 e.target.offsetLeft를 통해 쉽게 가져올 수 있었다.
<br/>


## 2. 커뮤니티 페이지
### - 기간 : 2022. 12. 13. ~ 2022. 12. 27.
## 
> ### 핵심 기능
>- 변형된 Carousel, Ck-Editor*, Pagination* library
>- database time 출력 moment.js timezone 설정
>- redux를 활용한 게시글 이슈태그 목록 출력/전환 기능
>- map과 삼항 연산자를 활용한 다소 복잡한 조건부 component 구조 생성
>- 유저가 작성한 태그를 일정한 조건의 split을 통해 #태그로 가공 처리
>- 게시글 상세 route - board/:boardId로 페이지 서치및 링크 구현

>* 바나나 사진 올리기 - 해당 구현사항 이미지와 설명으로 변경하기~
...
>- pagination ui를 수정해 게시글 목록과 어우러지게함
...
>- ckeditor의 ui를 수정해 world, title, tags 입력과 어우러지게함



> #### # 게시글 상세 페이지
>- Carousel 변형 슬라이드 : 마우스로 끌어 넘기거나 점 클릭해 볼수 있고 1초마다 자동 넘어감, Carousel slide의 ui를 수정해 게시글 목록및 tags ui와 어우러지게함
>- 게시글 이슈태그 : 게시글의 공감수 높은 순 정렬해 맨 처음 태그 가져옴, 태그 클릭시 해당 페이지 이동, 이슈태그 내용 작성하고 엔터시 해당 검색 페이지로 이동함
>- heart 버튼 : 클릭시 imags의 left를 조절해 하트가 터지는 모션을 보여줌
