...
const DetailComponent = () => {
  ...
  const commentReq = axios.post(
    "http://localhost:8080/api/comment/getComment",
    {
      boardId: boardId,
    }
  );
  ...
  // 랜더링 이후 값을 집어넣어줌
  useEffect(() => {
    const boardReq = axios.post(
      "http://localhost:8080/api/board/eyeCountUpdate",
      {
        boardId: boardId,
      }
    );
          ...
          <BoardInfo>
            ...
            <IconInfo>
              <span style={{ margin: "0px 10px" }}>
                <img src={eyeImg} alt={"조회 아이콘"} /> {board?.eyeCount}{" "}
              </span>
              <span>
                <img src={dateImg} alt={"시간 아이콘"} />{" "}
                {moment(board?.updatedAt, "YYYY-MM-DDTHH:mm:ssZ")
                  .toDate()
                  .toLocaleString()
                  .slice(
                    0,
        ...
        <LikeWrap>
          <LikeBtn
            onClick={async (e) => {
              ...
              const likeCountUpReq = await axios.post(
                "http://localhost:8080/api/board/likeCountUpdate",
                {
                  boardId: boardId,
                }
              );
                ...
                <UpDelBtn
                  onClick={async () => {
                    ...
                    if (deleteConfirm) {
                      ...
                      await axios.post(
                        "http://localhost:8080/api/board/destroy",
                        {
                          boardId: board.id,
                        }
                      );
              ...
              return (
                <Comment key={`comment-${idx}`}>
                  ...
                  <CommentUserInfo key={`commentUserInfo-${idx}`}>
                    ...
                    <CommentTimeSpan key={`createTime-${idx}`}>
                      {moment(comment.createdAt, "YYYY-MM-DDTHH:mm:ssZ")
                        .toDate()
                        .toLocaleString()
                        .slice(
                          0,
                  ...
                  {/* 댓글내용 */}
                  <CommentValue key={`commentText-${idx}`}>
                    {comment.text}
                  </CommentValue>
            ...
            <CommentBtnWrap>
              ...
              <CommentAddBtn
                onClick={async () => {
                  ...
                  // 서버쪽에 등록 요청을 보냄
                  const commentAddRed = await axios.post(
